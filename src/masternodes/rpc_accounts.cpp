...
UniValue getaccount(const JSONRPCRequest& request) {
    ...
    CCustomCSView mnview(*pcustomcsview);
    auto targetHeight = WITH_LOCK(cs_main, return ::ChainActive().Height()) + 1;
...
UniValue listaccounthistory(const JSONRPCRequest& request) {
    ...
    uint32_t height = chainHeight(*pwallet->chain().lock());
...
UniValue accounthistorycount(const JSONRPCRequest& request) {
    ...
    CScript lastOwner;
    uint64_t count = 0;
    uint32_t lastHeight = chainHeight(*pwallet->chain().lock());
