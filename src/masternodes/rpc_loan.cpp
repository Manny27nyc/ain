...
UniValue getcollateraltoken(const JSONRPCRequest& request) {
    ...
    uint32_t height = WITH_LOCK(cs_main, return ::ChainActive().Height());
...
UniValue getloaninfo(const JSONRPCRequest& request) {
    ...
    pcustomcsview->ForEachVaultCollateral([&, height = height, blockTime = blockTime](const CVaultId& vaultId, const CBalances& collaterals) {
        LogPrint(BCLog::LOAN,"getloaninfo()->Vault(%s):\n", vaultId.GetHex());
        auto rate = pcustomcsview->GetLoanCollaterals(vaultId, collaterals, height, blockTime, useNextPrice, requireLivePrice);
        if (rate)
        {
...
UniValue getinterest(const JSONRPCRequest& request) {
    ...
    UniValue ret(UniValue::VARR);
    uint32_t height = WITH_LOCK(cs_main, return ::ChainActive().Height()) + 1;
