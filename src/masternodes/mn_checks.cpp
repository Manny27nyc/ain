...
Res CPoolSwap::TextExecuteSwap(CCustomCSView& view) {

    std::vector<std::vector<DCT_ID>> poolPaths = CalculatePoolPaths(view);

    std::pair<Res, CAmount> bestResult{Res::Ok(), 0};
    // Loop through all common pairs
    for (const auto& path : poolPaths) {
        ...
        // Execute pool path
        Res swapResult = ExecuteSwap(dummy, path);

        // Add error for RPC user feedback
        if (!swapResult) {
            const auto token = dummy.GetToken(currentID);
            if (token) {
                errors.emplace_back(token->symbol, swapResult.msg);
            }
            break;
        }

        if (result > bestResult.second){
            bestResult = {swapResult, result};
        }
    }
