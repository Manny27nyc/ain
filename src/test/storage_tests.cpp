...
{
    {
        ...
        auto rawFlushable2 = flushable2.GetRaw();
        it = NewKVIterator<TestForward>(TestForward{0}, rawFlushable2);
        ...
        it = NewKVIterator<TestForward>(TestForward{2}, rawFlushable2);
        BOOST_CHECK(it.Valid());
        BOOST_CHECK(it.Value().as<int>() == 9);
        it.Prev();
        BOOST_CHECK(it.Valid());
        ...
        auto rawFlushable3 = flushable3.GetRaw();
        it = NewKVIterator<TestForward>(TestForward{0}, rawFlushable3);
        BOOST_CHECK(!it.Valid());
    }
...
{
    {
        ...
        count = 0;
        // view contains all changes after pcustomcsview flush
        view.ForEach<TestForward, TestForward, int>([&](TestForward key, int value) {
